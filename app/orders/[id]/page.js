...
export default function OrderDetailPage() {
    ...
    // ── Timeline from new API response ──
    // stages = { order_id, order_number, current_stage_id, timeline: [...] }
    const timeline = stages?.timeline ?? [];
    const currentStageId = stages?.current_stage_id ?? null;
                        ...
                        {timeline.length > 0 ? (
                            timeline
                                ...
                                .map((stage, idx, arr) => {
                                    ...
                                    // connector line color: filled if this stage AND next stage are both completed
                                    const nextCompleted = !isLast && arr[idx + 1]?.completed;
                                    return (
                                        <div key={stage.stage_id} className="flex gap-4">
                                            <div className="flex flex-col items-center">
                                                ...
                                                {!isLast && (
                                                    <div className={`w-0.5 flex-1 my-1 min-h-[20px] ${nextCompleted ? 'bg-[#ed1c24]' : 'bg-gray-200'}`} />
                                                )}
                                            </div>
                                            <div className="pb-5">
                                                ...
                                                {stage.timeline_date && (
                                                    <p className="text-xs text-gray-400 mt-0.5">
                                                        {formatDate(stage.timeline_date)}
                                                    </p>
                                                )}
                                                {isCurrent && stage.notes && (
                                                    <p className="text-xs text-gray-400 mt-0.5 italic">{stage.notes}</p>
                                                )}
                                                {!isCurrent && stage.notes && (
                                                    <p className="text-xs text-gray-400 mt-0.5 italic">{stage.notes}</p>
                                                )}
