...
const DetailPanel = memo(({
    ...
    return (
        ...
        >
            ...
            <div className=''>
                {(() => {
                    const totalSelected = Object.values(hatQuantities?.[uniqueHatId] || {})
                    ...
                    return (
                        <div className={`mx-6 mb-4 rounded-xl px-5 py-3 flex items-center gap-3 border transition-all duration-300
            ${isComplete
                            ...
                            {isComplete ? (
                                ...
                                <MdAddShoppingCart className="text-orange-400 text-xl flex-shrink-0" />
                            )}
                    ...
                    ${isComplete ? 'text-green-700' : 'text-orange-700'}`}>
                                    {isComplete
                                        ...
                                        : `${totalSelected} selected — ${24 - totalSelected} more needed`}
                                </span>
                                <span className={`text-xs font-normal mt-0.5
                    ${isComplete ? 'text-green-500' : 'text-orange-400'}`}>
                                    {isComplete
                                        ? 'You can add as many more as you like'
                                        : 'Minimum 24 required per hat style'}
                                </span>
                            </div>
                        </div>
                    );
...
const ProductAccordion = ({ selectedDecoName, selectedDecoId, hatQuantities: initialHatQuantities, setHatQuantities: setParentHatQuantities }) => {
        ...
        brandList?.data?.forEach((brand) => {
            ...
            hats.forEach((hat) => {
                ...
                if (totalQty > 0 && totalQty < 24) {
                    invalidHats.push(`${hat.name} (${totalQty}/24)`);
                }
            });
        });
    ...
    return (
        <div className='product_details_area max-w-[1400px] mx-auto p-4'>
            ...
            <div className='fixed top-[80px] md:top-[100px] left-1/2 -translate-x-1/2 z-10 w-max'>
                <button
