...
export function useCartActions({ open } = {}) {
  ...
  const queueUpdate = async (item, newQty, oldQty) => {
  ...
  // ─── INCREASE ─────────────────────────────────────────────────────────────
  // ✅ async — QtyInput এ await করলে loader সঠিকভাবে কাজ করবে
  const handleIncrease = async (item) => {
    await queueUpdate(item, item.quantity + 1, item.quantity);
  };

  // ─── DECREASE ─────────────────────────────────────────────────────────────
  // ✅ async
  const handleDecrease = async (item) => {
    if (item.quantity <= 1) { await handleDelete(item); return; }
    await queueUpdate(item, item.quantity - 1, item.quantity);
  };
  ...
  const handleManualChange = async (item, newQty) => {
    if (newQty === 0) { await handleDelete(item); return; }
    await queueUpdate(item, newQty, item.quantity);
  };
