...
// Stays in sync with item.quantity from parent, but lets user type freely.
// Commits on blur or Enter. Reverts if invalid.
...
const CartDropdown = ({ open, onClose }) => {
  ...
  // ─── LOCAL STABLE STATE ───────────────────────────────────────────────────
  // Render from localItems, NOT directly from Redux cartGroups.
  // Only do a full sync when item count changes (add/delete).
  // On qty updates: update in-place → no reordering.
  ...
  useEffect(() => {
    ...
    if (isFirstLoad.current) {
      ...
      if (incomingIds !== localIds) {
        // Structural change (item added/deleted) → full sync
        setLocalItems(incoming);
      }
      // qty-only change → do nothing here, local state already updated optimistically
    }
